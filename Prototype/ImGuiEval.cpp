...
bool ImGuiEval::initialize( StateDb& sdb, Assets& assets )
{
    ...
    {
        u32 fontTextureAsset =
            assets.asset( "Procedural/Textures/ImGuiFont", Assets::Flag::PROCEDURAL | Assets::Flag::DYNAMIC );
        ...
        memcpy(
            &texture->pixels[ 0 ], imGuiPixels, texture->pixels.size() * sizeof( Assets::Texture::Pixel ) );
...
void ImGuiEval::update( StateDb& sdb, Assets& assets, Renderer& renderer, double deltaTimeInS )
{
        ...
        if ( m_profilerVisible ) {
            ...
            if ( mainThread ) {
                ...
                for ( Profiler::SectionSample& sample : mainThread->samples ) {
                    ...
                    ImGui::Text(
                        "%8.0f us",
                        Profiler::instance()->ticksToMs( sample.ticksExit - sample.ticksEnter ) * 1000.0f );
                    if ( maxCallDepth >= 0 ) {
                        continue;
                    }
    ...
    // Retrieve render command buffers and transform/prepare for renderer
    {
        ...
        for ( int cmdListIdx = 0; cmdListIdx < drawData->CmdListsCount; ++cmdListIdx ) {
            ...
            for ( auto& attr : model->attrs )
                attr.data = dataPointer;
            ...
            for ( int cmdIdx = 0; cmdIdx < cmdCount; ++cmdIdx ) {
                ...
                part.scissor      = glm::u16vec4(
                    ...
                    u16( cmd.ClipRect.z - cmd.ClipRect.x ), u16( cmd.ClipRect.w - cmd.ClipRect.y ) );
