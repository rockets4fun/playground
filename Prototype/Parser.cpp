...
bool Parser::advance()
{
    ...
    if (!m_tokenBegin) m_tokenBegin = m_begin;
    else m_tokenBegin = m_tokenEnd;

    while (s_charTypes[*m_tokenBegin] == CharType::WHITESPACE
        && m_tokenBegin < m_end) ++m_tokenBegin;

    m_tokenEnd = m_tokenBegin;
    m_tokenCharType = s_charTypes[*m_tokenBegin];
    while (s_charTypes[*m_tokenEnd] == m_tokenCharType && m_tokenEnd < m_end) ++m_tokenEnd;
