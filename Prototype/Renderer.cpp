...
static const int attrSize[] = { 4, 1, 2, 4 };

static const GLenum attrGlType[] = { GL_FLOAT, GL_UNSIGNED_BYTE, GL_UNSIGNED_SHORT, GL_UNSIGNED_INT };
...
{
    typedef void( APIENTRYP GetProc )( GLuint shader, GLenum pname, GLint* params );
    typedef void( APIENTRYP InfoLogProc )( GLuint object, GLsizei bufSize, GLsizei* length, GLchar* infoLog );

    PrivateHelpers( PrivateFuncs* funcs );

    bool updateShader( GLuint& shader, GLenum type, const std::string& source );
    bool updateProgram(
        Program::PrivateInfo* programPrivate, const std::map< std::string, GLuint >& attrIndicesByName );
...
// -------------------------------------------------------------------------------------------------
Renderer::PrivateHelpers::PrivateHelpers( PrivateFuncs* funcs )
    : funcs( funcs )
{
}

// -------------------------------------------------------------------------------------------------
bool Renderer::PrivateHelpers::updateShader( GLuint& shader, GLenum type, const std::string& src )
{
    if ( !shader ) {
    ...
    Program::PrivateInfo* programPrivate, const std::map< std::string, GLuint >& attrIndicesByName )
{
    ...
    for ( auto& attrIndexMapEntry : attrIndicesByName ) {
        funcs->glBindAttribLocation(
            programPrivate->program, attrIndexMapEntry.second, attrIndexMapEntry.first.c_str() );
    }
...
void Renderer::registerTypesAndStates( StateDb& sdb )
{
    ...
    Program::Info::STATE = sdb.registerState( Program::TYPE, "Info", sizeof( Program::Info ) );
    Program::PrivateInfo::STATE =
        sdb.registerState( Program::TYPE, "PrivateInfo", sizeof( Program::PrivateInfo ) );

    Mesh::TYPE               = sdb.registerType( "Mesh", 4096 );
    Mesh::Info::STATE        = sdb.registerState( Mesh::TYPE, "Info", sizeof( Mesh::Info ) );
    Mesh::PrivateInfo::STATE = sdb.registerState( Mesh::TYPE, "PrivateInfo", sizeof( Mesh::PrivateInfo ) );

    Texture::TYPE        = sdb.registerType( "Texture", 256 );
    Texture::Info::STATE = sdb.registerState( Texture::TYPE, "Info", sizeof( Texture::Info ) );
    Texture::PrivateInfo::STATE =
        sdb.registerState( Texture::TYPE, "PrivateInfo", sizeof( Texture::PrivateInfo ) );

    Camera::TYPE        = sdb.registerType( "Camera", 8 );
    Camera::Info::STATE = sdb.registerState( Camera::TYPE, "Info", sizeof( Camera::Info ) );

    Pass::TYPE        = sdb.registerType( "Pass", 8 );
    Pass::Info::STATE = sdb.registerState( Pass::TYPE, "Info", sizeof( Pass::Info ) );
...
bool Renderer::initialize( StateDb& sdb, Assets& assets )
{
    ...
    */
    ...
    // Set up frame buffer for playing around...
    {
        ...
        funcs->glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, 400, 225, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0 );
        ...
        funcs->glTexImage2D(
            GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, 400, 225, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE, 0 );
        funcs->glBindTexture( GL_TEXTURE_2D, 0 );
        // Frame buffer
        funcs->glGenFramebuffers( 1, &state->defFbo );
        funcs->glBindFramebuffer( GL_FRAMEBUFFER, state->defFbo );
        funcs->glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, state->colorTex, 0 );
        funcs->glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, state->depthTex, 0 );
...
void Renderer::update( StateDb& sdb, Assets& assets, Renderer& renderer, double deltaTimeInS )
{
    ...
    */
    ...
    for ( auto& meshMapEntry : state->meshesByModelAsset ) {
        ...
        for ( auto& attr : privateMesh->asset->attrs ) {
            ...
            funcs->glVertexAttribPointer(
                idx, GLint( attr.count ), attrGlType[ attr.type ], attr.normalize,
                GLsizei( vboInfo.vertexStrideInB ), (GLvoid*)attr.offsetInB );
    ...
    for ( auto programPrivate : programsPrivate ) {
        ...
        helpers->updateShader(
            programPrivate->vertexShader, GL_VERTEX_SHADER,
            programAsset->sourceByType[ Assets::Program::VERTEX_SHADER ] );
        helpers->updateShader(
            programPrivate->fragmentShader, GL_FRAGMENT_SHADER,
        ...
        programPrivate->uDiffuseMul = funcs->glGetUniformLocation( programPrivate->program, "DiffuseMul" );
        programPrivate->uAmbientAdd = funcs->glGetUniformLocation( programPrivate->program, "AmbientAdd" );
        programPrivate->uRenderParams =
            funcs->glGetUniformLocation( programPrivate->program, "RenderParams" );
        programPrivate->uColorTex = funcs->glGetUniformLocation( programPrivate->program, "ColorTex" );
        programPrivate->uDepthTex = funcs->glGetUniformLocation( programPrivate->program, "DepthTex" );

        programPrivate->uTexture0 = funcs->glGetUniformLocation( programPrivate->program, "Texture0" );
        programPrivate->uTexture1 = funcs->glGetUniformLocation( programPrivate->program, "Texture1" );
        ...
        /*
        // DEBUG: Query actual attribute locations
    ...
    for ( auto texturePrivate : texturesPrivate ) {
        ...
        funcs->glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
        funcs->glTexImage2D(
            GL_TEXTURE_2D, 0, GL_RGBA, texturePrivate->asset->width, texturePrivate->asset->height, 0,
            GL_RGBA, GL_UNSIGNED_BYTE, &texturePrivate->asset->pixels[ 0 ] );
        funcs->glBindTexture( GL_TEXTURE_2D, 0 );
...

    {
        ...
        glm::fmat4 projection = glm::perspective( glm::radians( 30.0f * aspect ), aspect, 0.5f, 200.0f );
        glm::fmat4 worldToView;
        if ( activeCameraHandle ) {
            auto camera = sdb.state< Camera::Info >( activeCameraHandle );
            worldToView = glm::lookAt( camera->position, camera->target, glm::fvec3( 0.0f, 0.0f, 1.0f ) );
        }
...
void Renderer::renderPass(
    StateDb& sdb, u32 renderMask, const Program::PrivateInfo* programPrivate, const glm::fmat4& projection,
    const glm::fmat4& worldToView, const glm::fvec4& renderParams )
{
    funcs->glUseProgram( programPrivate->program );
    ...
    for ( auto mesh : meshes ) {
        ...
        if ( privateMesh->flags & PrivateMesh::Flag::DIRTY ) {
            ...
            for ( auto& vbosIt : privateMesh->vbosByInitialData ) {
                ...
                /*
                // DEBUG: Overwrite vertex attributes
                if (vbosIt.second.vertexStrideInB == 20)
                {
                    for (int vertexIdx = 0; vertexIdx < vertexCount; ++vertexIdx)
                    {
                        uint8_t* colorData = ((uint8_t*)data) + 20 * vertexIdx + 16;
                        colorData[0] = 128;
                        colorData[1] = 0;
                        colorData[2] = 0;
                        colorData[3] = 128;
                        float* uvData = (float*)(((uint8_t*)data) + 20 * vertexIdx + 8);
                        uvData[0] = 1.0f;
                        uvData[1] = 1.0f;
                    }
                }
        ...
        glm::fmat4 translation = glm::translate( glm::fmat4( 1.0f ), mesh->translation );
        glm::fmat4 rotation    = glm::mat4_cast( mesh->rotation );

        glm::fmat4 modelToWorld = translation * rotation;
        if ( mesh->flags & Mesh::Flag::SCALED ) {
            glm::fmat4 scale = glm::scale( glm::fmat4( 1.0f ), mesh->scale );
            modelToWorld     = modelToWorld * scale;
        }
        ...
        if ( mesh->flags & Mesh::Flag::DRAW_PARTS ) {
            if ( privateMesh->ibo ) {
                ...
                for ( auto& part : privateMesh->asset->parts ) {
                    if ( activeMaterialHint != part.materialHint ) {
                        ...
                        if ( texture )
                            funcs->glBindTexture( GL_TEXTURE_2D, texture->texture );
                        else
                            funcs->glBindTexture( GL_TEXTURE_2D, 0 );
                    ...
                    if ( activeScissor != part.scissor ) {
                        ...
                        if ( activeScissor.z && activeScissor.w ) {
                            funcs->glScissor(
                                activeScissor.x, activeScissor.y, activeScissor.z, activeScissor.w );
                    ...
                    funcs->glDrawElements(
                        GL_TRIANGLES, GLint( part.count ), privateMesh->iboGlType, indices );
                }
            ...
            if ( privateMesh->ibo ) {
                funcs->glDrawElements(
                    GL_TRIANGLES, GLsizei( privateMesh->indexCount ), privateMesh->iboGlType, (void*)0 );
            }
            else {
