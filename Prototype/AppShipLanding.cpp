...
// -------------------------------------------------------------------------------------------------
void pushRect2d(
    Assets::Model* model, const glm::fvec2& ll, const glm::fvec2& ur,
    const glm::fvec3& color = glm::fvec3( 1.0f, 1.0f, 1.0f ), float z = 0.0f )
{
    ...
    // Lower left triangle
    model->positions.push_back( glm::fvec3( ll.x, ll.y, z ) );
    model->diffuse.push_back( color );
    model->positions.push_back( glm::fvec3( ur.x, ur.y, z ) );
    model->diffuse.push_back( color );
    model->positions.push_back( glm::fvec3( ll.x, ur.y, z ) );
    model->diffuse.push_back( color );
    ...
    model->positions.push_back( glm::fvec3( ll.x, ll.y, z ) );
    model->diffuse.push_back( color );
    model->positions.push_back( glm::fvec3( ur.x, ll.y, z ) );
    model->diffuse.push_back( color );
    // Others...
    for ( int nIdx = 0; nIdx < 6; ++nIdx )
        model->normals.push_back( glm::fvec3( 0.0f, 0.0f, 1.0f ) );
    for ( int nIdx = 0; nIdx < 6; ++nIdx )
        model->ambient.push_back( glm::fvec3( 0.0f, 0.0f, 0.0f ) );
}

// -------------------------------------------------------------------------------------------------
void pushRectOutline2d(
    Assets::Model* model, double t, const glm::fvec2& ll, const glm::fvec2& ur, const glm::fvec3& color,
    float z = 0.0f )
{
...
bool AppShipLanding::initialize( StateDb& sdb, Assets& assets )
{
    ...
    m_oceanModelAsset =
        assets.asset( "Procedural/Models/Ocean", Assets::Flag::PROCEDURAL | Assets::Flag::DYNAMIC );
    m_uiModelAsset = assets.asset( "Procedural/Models/Ui", Assets::Flag::PROCEDURAL | Assets::Flag::DYNAMIC );
    m_postModelAsset = assets.asset( "Procedural/Models/Post", Assets::Flag::PROCEDURAL );
    ...
    }
    {
        ...
        pushRect2d( model, glm::fvec2( 0.0f, 0.0f ), glm::fvec2( 800.0f, 450.0f ) );
    ...
    // Create simplistic tiled ocean
    {
        ...
        for ( int y = 0; y < vertexCount - 1; ++y ) {
            for ( int x = 0; x < vertexCount - 1; ++x ) {
                ...
                for ( int vIdx = 0; vIdx < 6; ++vIdx )
                    model->diffuse.push_back( color );
                for ( int vIdx = 0; vIdx < 6; ++vIdx )
                    model->ambient.push_back( glm::fvec3( 0.0 ) );
            }
        }
        model->setDefaultAttrs();
        ...
        for ( int y = 0; y < tileCount; ++y ) {
            for ( int x = 0; x < tileCount; ++x ) {
                ...
                mesh->translation = glm::fvec3(
                    glm::fvec2( float( x ), float( y ) ) * unitSize - 0.5f * float( tileCount ) * unitSize,
                    0.0f );
                mesh->groups = Renderer::Group::DEFAULT;
            }
        }
        ...
        for ( int col = 0; col < PLATFORM_SPHERE_COUNT; ++col ) {
            for ( int row = 0; row < PLATFORM_SPHERE_COUNT; ++row ) {
                glm::fvec3 translation = glm::fvec3(
                    -0.5f * platformSize + float( col ) / float( PLATFORM_SPHERE_COUNT - 1 ) * platformSize,
                    -0.5f * platformSize + float( row ) / float( PLATFORM_SPHERE_COUNT - 1 ) * platformSize,
                    0.0 );
    ...
    for ( int meshIdx = 0; meshIdx < 150; ++meshIdx ) {
        ...
        mesh->translation =
            glm::linearRand( glm::fvec3( -15.0f, -15.0f, +20.0f ), glm::fvec3( +15.0f, +15.0f, +40.0f ) );
        mesh->groups = Renderer::Group::DEFAULT;
        ...
        if ( enableRocket && meshIdx == 0 ) {
            ...
            for ( const Assets::Model::Instance& instance : model->instances ) {
                ...
                Math::decomposeTransform( xform, thruster->translation, thruster->rotation, thruster->scale );
                ...
                if ( thruster->scale.x < 0.0 || thruster->scale.y < 0.0 || thruster->scale.z < 0.0 ) {
...
void AppShipLanding::update( StateDb& sdb, Assets& assets, Renderer& renderer, double deltaTimeInS )
{
    ...
    // Update thruster debug meshes
    {
        ...
        for ( auto thruster : thrusters ) {
            ...
            debugMesh->translation = shipMesh->translation + shipMesh->rotation * thruster->translation;
    ...
    // Update tileable dynamic ocean model
    {
        ...
        for ( int vertexIdx = 0; vertexIdx < vertexCount; ++vertexIdx ) {
            ...
            model->normals[ vertexIdx ] =
                glm::normalize( glm::cross( oceanPtRight - oceanPtLeft, oceanPtAbove - oceanPtBelow ) );
        }
    }
    ...
    // Update camera
    {
        ...
        if ( glm::abs( horizontalRotationInDeg ) > 0.001 ) {
            xform = glm::rotate(
                xform, glm::radians( float( horizontalRotationInDeg ) ), glm::fvec3( 0.0f, 0.0f, 1.0f ) );
        }
        if ( glm::abs( verticalRotationInDeg ) > 0.001 ) {
            xform =
                glm::rotate( xform, glm::radians( float( verticalRotationInDeg ) ), verticalRotationAxis );
        }
        xform            = glm::translate( xform, -camera->target.xyz() );
    ...
    if ( m_pusherAffectorHandle ) {
        ...
        */
        ...
        arrowMesh->rotation    = Math::rotateFromTo( glm::fvec3( 0.0f, 1.0f, 0.0f ), -affector->force );

        // Emergency motors off...
        double errorAngleInDeg = glm::degrees( glm::angle( Math::rotateFromTo( nominalDir, actualDir ) ) );
        ...
        while ( m_rocketSmokeParticlesDelay < 0.0 ) {
            ...
            if ( rand() % 100 > 90 )
                ...
                particle->maxSize = 5.0f + glm::linearRand( -0.8f, +0.25f );
        ...
        for ( auto particle : particles ) {
            ...
            if ( life >= 1.0f - shrink ) {
                ...
                uniformScale = particle->minSize
                    + glm::smoothstep( 0.0f, grow, life ) * ( particle->maxSize - particle->minSize );
                mesh->diffuseMul = glm::mix(
                    glm::fvec4( 1.0, 0.5, 0.0, 1.0 ), glm::fvec4( 0.8, 0.8, 0.8, 1.0 ), life / grow );
                mesh->ambientAdd = glm::mix(
                    glm::fvec4( 1.0, 0.5, 0.0, 0.0 ), glm::fvec4( 0.0, 0.0, 0.0, 0.0 ), life / grow );
            }
            else {
                uniformScale =
                    particle->maxSize - glm::smoothstep( grow, 1.0f - shrink, life ) * particle->minSize;
            }
    ...
    // Update profiling UI
    {
        ...
        for ( int msIdx = 0; msIdx < 16; ++msIdx ) {
            pushRectOutline2d(
                uiModel, outlPx, glm::fvec2( offsPx + msPx * ( msIdx + 0 ) + outlPx, offsPx ),
                glm::fvec2( offsPx + msPx * ( msIdx + 1 ), offsPx + barPx ), black, 5.0f );
        }
        pushRectOutline2d(
            uiModel, outlPx * 2.0f, glm::fvec2( offsPx, offsPx - outlPx ),
...
float AppShipLanding::oceanEquation( const glm::fvec2& position, double timeInS )
{
    ...
    float result        = 0.0f;
    result += 0.50f * __sin( twoPi / ( 0.500f * unitSize.x ) * float( position.x + position.y + timeInS ) );
    result += 0.50f * __sin( twoPi / ( 1.000f * unitSize.x ) * float( position.x - position.y + timeInS ) );
    result +=
        0.10f * __sin( twoPi / ( 0.250f * unitSize.x ) * float( position.x + position.y - timeInS * 0.5f ) );
    return result;
}
#undef __sin

// -------------------------------------------------------------------------------------------------
void AppShipLanding::addBuoyancyAffector(
    StateDb& sdb, const glm::fvec3& translation, u64 parentRigidBodyHandle )
{
...
void AppShipLanding::updateBuoyancyAffectors( StateDb& sdb, double timeInS )
{
    ...
    for ( auto buoyancyAffectorHandle : m_buoyancyAffectorHandles ) {
        ...
        glm::fvec3 affectorPosGlobal = mesh->translation + mesh->rotation * affector->forcePosition;
        ...
        if ( spherePenetrationDepth > 0.0 ) {
            spherePenetrationVolume = ( 1.0 / 3.0 ) * glm::pi< double >()
                * glm::pow( spherePenetrationDepth, 2.0 ) * ( 3.0 * sphereRadius - spherePenetrationDepth );
        }
        ...
        // Force from friction between moving RB and water
        {
            glm::fvec3 velocityAtAffector = rigidBody->linearVelocity
                + glm::cross( rigidBody->angularVelocity, mesh->rotation * affector->forcePosition );
